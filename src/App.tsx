...
import { useFullscreenDocument, useHotkeys, useLocalStorage, useMediaQuery } from '@mantine/hooks'
...
import { clearCurrentUser, getDomainActorId, persistCurrentUser } from '@auth/currentUser'
...
function Main({ onLoginUser, onLogoutUser, user, setUser }: MainProps) {
  const { toggle } = useFullscreenDocument()
  ...
  const { loginWithContext, logout, username, isAuthenticated, isAuthenticating, isInitializing, error, accessToken, clearError, context } =
    useArchbaseAuthenticationManager({})
  ...
  const getUserInfo = useCallback(async () => {
    ...
    try {
      ...
      // Usar o contexto de autenticação se disponível
      if (context && typeof context === 'object' && 'userId' in context) {
        const appContext = context as any
        onLoginUser(
          new AppUser({
            userId: appContext.userId,
            id: appContext.userId || username,
            displayName: appContext.fullName || username,
            fullName: appContext.fullName,
            email: appContext.email || username,
            phone: appContext.phone,
            photo: appContext.avatarUrl,
            role: appContext.role,
            status: appContext.status,
            permissions: appContext.permissions || [],
            isActive: appContext.isActive,
            teamId: appContext.teamId,
            teamName: appContext.teamName,
            territoryId: appContext.territoryId,
            territoryName: appContext.territoryName,
            isAdmin: appContext.role === 'ADMIN',
          })
        )
        setUserInfoLoaded(true)
      } else {
        // Fallback: buscar dados do Archbase UserService
        try {
  ...
  }, [accessToken, username, context, usuarioServiceApi, isGettingUser, userInfoLoaded, onLoginUser, logout, onLogoutUser])
